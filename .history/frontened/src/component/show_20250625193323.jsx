			...
			<div className="w-fit flex flex-wrap gap-4 sm:gap-4 md:gap-5 p-4 sm:p-2 md:p-6">	
				{data.map((item, idx) =>
					Object.entries(item).map(([key, value]) => {
						...
						<div
							key={`${idx}-${key}`}
							className="relative group w-25 h-25 sm:w-40 sm:h-40 md:w-40 md:h-40 border-2 shadow-xl rounded overflow-hidden"
							>
							<img
								src={src}
								alt={key}
								className="scale-80 w-full h-full object-cover"
							/>
