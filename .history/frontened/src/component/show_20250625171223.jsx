			...
			<div className="w-fit flex flex-wrap gap-4 sm:gap-4 md:gap-5 p-4 sm:p-2 md:p-6">	
				{data.map((item, idx) =>
					Object.entries(item).map(([key, value]) => {
						...
						<div
							key={`${idx}-${key}`}
							className="relative group w-25 h-25 sm:w-40 sm:h-40 md:w-40 md:h-40 border rounded overflow-hidden"
							>
							<img
								src={src}
								alt={key}
			...
			<div className="w-full bg-zinc-200">
				...
				<div className="fixed inset-0 h-full w-full bg-white/40 backdrop-blur flex items-center justify-center p-2 md:p-4 z-50">
					<div className="relative bg-white rounded-lg shadow-lg md:p-4">
						...
						<button
						onClick={() => setShow_img('')}
						className="absolute top-2 right-2 bg-red-600 hover:bg-red-700 text-white p-2 rounded-sm text-sm font-bold border-b-4 border-r-2 border-red-800 cursor-pointer"
						>
