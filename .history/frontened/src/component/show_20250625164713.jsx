...
function DataShowcaseOptions({ data }) {
  ...
  <div className="flex flex-wrap justify-center gap-2 bg-zinc-400 max-w-full">
    {data.map((item, idx) =>
      Object.entries(item).map(([key, value]) => {
        ...
        return src ? (
          <div
            key={`${idx}-${key}`}
            className="w-24 h-24 sm:w-40 sm:h-40 md:w-40 md:h-40 border rounded overflow-hidden"
          >
            <img
              src={src}
              alt={key}
