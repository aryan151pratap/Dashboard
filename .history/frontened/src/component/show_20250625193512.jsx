			...
			<div className="w-fit flex flex-wrap gap-4 sm:gap-4 md:gap-5 p-4 sm:p-2 md:p-6">	
				{data.map((item, idx) =>
					Object.entries(item).map(([key, value]) => {
						...
						<div
							key={`${idx}-${key}`}
							className="hover:scale-90 relative group w-25 h-25 sm:w-40 sm:h-40 md:w-40 md:h-40 border-2 rounded overflow-hidden transform transition-transform duration-300"
							>
							<img
								src={src}
								alt={key}
