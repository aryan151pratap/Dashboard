...
function Filter({ collection }) {
	...
	function getKeysAtExactPath(data, path) {
		const keysSet = new Set();

		for (let obj of data) {
			let current = getValueByPath(obj, path);
		...
		<div className="w-full flex flex-col gap-2">
			<div className="w-full flex flex-col sm:flex-row flex-wrap gap-2 p-2 bg-zinc-400">
				{keyPath.map((_, level) => {
					...
					const options = getKeysAtExactPath(data, parentPath);
					return (
						<select
							key={level}
				...
				{(() => {
					const options = getKeysAtExactPath(data, keyPath);
					if (options.length > 0) {
						return (
							<select
								className="p-2 bg-zinc-200"
