...
function DataShowcaseOptions({ data }) {
	...
	const renderImages = () => (
		<div className="w-full items-center justify-between flex flex-wrap gap-2 p-4 sm:p-2">
			<div className="w-full flexs">
				{data.map((item, idx) =>
					Object.entries(item).map(([key, value]) => {
						if (typeof value !== "string") return null;

						const isPureBase64 = value.length > 100 && /^[A-Za-z0-9+/=]+$/.test(value);
						const src = isPureBase64 ? `data:image/jpeg;base64,${value}` : null;
