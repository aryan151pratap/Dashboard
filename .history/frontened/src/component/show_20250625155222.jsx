		...
		<div className="items-center justify-between flex flex-wrap gap-2 p-4 sm:p-2">
			{data.map((item, idx) =>
				Object.entries(item).map(([key, value]) =>
					typeof value === "string" &&
					(value.startsWith("data:image") || /\.(jpg|jpeg|png|gif|webp)$/i.test(value) || value.includes("base64")) ? (
						<div key={`${idx}-${key}`} className="w-25 h-25 sm:w-40 sm:h-40 md:w-40 md:h-40  border rounded overflow-hidden">
							<img
								src={value.startsWith("data:image") ? value : `data:image/jpeg;base64,${value}`}
								alt={key}
