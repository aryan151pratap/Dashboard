...
function DataShowcaseOptions({ data }) {
	...
	const renderJSON = () => (
		<div className="bg-black text-green-400 p-2 text-sm overflow-auto max-h-[500px]">
			{data.map((item, i) => (
				<pre key={i} className="border-b border-gray-700">{JSON.stringify(item, null, 2)}</pre>
			))}
		</div>
	);
			...
			{data.map((item, idx) =>
				Object.entries(item).map(([key, value]) =>
					...
					(value.startsWith("data:image") || /\.(jpg|jpeg|png|gif|webp)$/i.test(value)) ? (
						<div key={`${idx}-${key}`} className="w-32 h-32 border rounded overflow-hidden">
							<img
								src={value}
								alt={key}
