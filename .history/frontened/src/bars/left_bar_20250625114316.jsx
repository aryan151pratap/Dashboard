		...
		<div className="sticky top-0 z-5 shrink-0 h-fit w-full pt-12 bg-zinc-200 border-b sm:border-r">
			...
			<div>
				<div className="flex flex-row border-b">
					<button className={`sm:text-center font-bold px-2 py-2 ${current_database === null ? 'text-white bg-zinc-700' : 'text-zinc-700'}`}
				...
				<ul className="text-sm flex flex-row overflow-x-auto">
				{current_database ? 
				<>	
					{current_database.collections?.map((i, index) => (
						<li key={index} className={`${current_collection === i ? 'bg-zinc-800 text-white font-semibold' : ''} flex hover:underline cursor-pointer px-4 py-2`}
						onClick={() => handle_current_collections(i)}
						>
							{i}
							{(loading && current_collection === i) &&
								<div className="ml-auto w-5 h-5 border-3 border-green-500 border-t-transparent rounded-full border-r animate-spin"></div>
							}
						</li>
					))}
				</>
				...
				<>
					{cluster.databases?.map((i, index) => (
						<li key={index} className="hover:underline cursor-pointer border-r px-4 py-2"
