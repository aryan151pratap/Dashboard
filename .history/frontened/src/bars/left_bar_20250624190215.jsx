...
function Left_bar({ setCollection, cluster }){
	...
	return(
		<>
		<div className="sticky top-0 shrink-0 h-fit sm:h-screen w-full sm:w-fit pt-12 bg-zinc-200 border-b sm:border-r">
			...
			<>	
				{current_database.collections?.map((i, index) => (
					<li key={index} className={`${current_collection === i ? 'bg-green-200 text-white font-semibold' : ''} flex hover:underline cursor-pointer border-r sm:border-r-0 sm:border-b px-4 py-2`}
					onClick={() => handle_current_collections(i)}
					>
						{i}
						{(loading && current_collection === i) &&
							<div className="ml-auto w-5 h-5 border-3 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
						}
					</li>
				))}
			</>
